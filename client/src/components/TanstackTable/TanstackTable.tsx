...
function TanstackTable({
  ...
  if (isError) {
    // RTK Query error can have different shapes
    let errorMessage = "Something went wrong";

    if ("status" in error) {
      // FetchBaseQueryError
      if (error.status === 500) errorMessage = "Server error, please try later";
      else if (error.status === 404) errorMessage = "Data not found";
      else errorMessage = "Error: " + JSON.stringify(error.data);
    } else {
      // SerializedError
