...
class ProductController {
      ...
      if (products.length > 0) {
        products = products.map((product) => {
          ...
          if (product.product_image) {
            productData.product_image_url = `http://localhost:3000/uploads/${product.product_image}`;
          }
  ...
  async getById(req, res) {
    try {
      ...
      if (product.product_image) {
        product.product_image_url = `http://localhost:3000/uploads/${product.product_image}`;
      }
